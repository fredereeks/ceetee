...
function SkillCard({image, title, text, direction}) {
  return (
    <aside className={`group py-8 px-4 flex flex-col items-stretch gap-3 md:gap-5 ${direction ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
        <div className="relative flex-1 mx-auto flex h-full w-full  py-4 md:w-[400px] flex-shrink-0 group-hover:shad bg-white rounded-md overflow-hidden">
            <img src={image} alt={title} className="h-full w-full rounded-md hover:shadow-xl shadow-lg object-cover md:object-cover scale-100 group-hover:scale-150" />
        </div>
