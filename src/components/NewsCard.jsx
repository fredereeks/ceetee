...
function NewsCard({id, image, title, text, comments, fullname, users, category, full}) {
  return (
    <Link to={`/news?${id}`} key={id} className={`relative overflow-hidden before:z-5 flex gap-2 p-2 sm:p-4 ${full ? 'w-full flex-row justify-start bg-white' : 'hover:shadow-2xl max-w-[25rem] flex-col justify-end shad before:w-full before:h-full before:absolute before:top-0 before:left-0 before:bg-gradient-to-t before:from-slate-800 before:to-transparent'} group rounded-base transition-all duration-300`}>
        <img src={image} alt={title} className={`${full ? 'relative' : 'absolute'} w-full h-full top-0 left-0 object-cover`} />
